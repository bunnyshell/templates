...
This Environment [Template](https://documentation.bunnyshell.com/docs/templates-what-are-templates) is a boilerplate for creating a new environment based on a stack using Java 17 with Spring Boot for the backend, Angular with Typescript for the frontend and MySQL as the database.
...
- `api` for backend, also based on a `eclipse-temurin` image
...
You need to provide the *remote path*: the path where code files are located within the container (you can find this out from the `Dockerfile`, it's in the `WORKDIR` statement); for the `api` application, it is `/app`.
...
```
$ bns remote-development up --port-forward "5005>5005"
? Local Path {YOUR_OWN_LOCAL_PATH}}
? Remote Path /app
...
And within the IDE terminal, you need to start the `jvm` with debugging capabilities:
```
/app # ./mvnw spring-boot:run -Dspring-boot.run.jvmArguments="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=5005"
```
